...
export function registerJsExtractorCommands(context: vscode.ExtensionContext): vscode.Disposable[] {
    const openCommand = vscode.commands.registerCommand('openhunt.jsextractor.openViewer', async (uri?: vscode.Uri) => {
...
async function listJsFiles(): Promise<{ name: string; path: string; relativePath: string }[]> {
                ...
                switch (message.command) {
                    case 'openFile':
                        const files = await vscode.window.showOpenDialog({
                            canSelectFiles: true,
                            canSelectMany: false,
                            filters: { 'JavaScript Files': ['js', 'jsx', 'ts', 'tsx', 'mjs', 'cjs'] }
                        });
                    ...
                    case 'listWorkspaceFiles':
                        const workspaceFiles = await listJsFiles();
                        panel?.webview.postMessage({
...
<head>
    ...
    <title>JS Endpoint Extractor</title>
</head>
<body>
    <div id="root" data-view="jsextractor"></div>
